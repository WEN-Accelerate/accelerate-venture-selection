      ...
      <div className="max-w-md w-full bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-100 p-8 relative z-10">
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={loading}
            className={`w-full py-3 px-4 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-semibold rounded-xl flex items-center justify-center gap-2 transition-all shadow-sm`}
          >
            Create Account
          </button>
        </div>
...
export default function App() {
  ...
  useEffect(() => {
    // 1. Initialize Netlify Identity Widget
    netlifyIdentity.init();

    // 2. Listen for auth state changes
    netlifyIdentity.on('login', (netlifyUser) => {
      console.log('Netlify Login:', netlifyUser);
      // Map Netlify user to our user object format
      const mappedUser = {
        uid: netlifyUser.id,
        email: netlifyUser.email,
        displayName: netlifyUser.user_metadata?.full_name,
        isAnonymous: false
      };
      setUser(mappedUser);
    ...
    // Check if duplicate listeners are an issue, likely not if this component mounts once.
    // However, cleaning up is good practice but netlifyIdentity is global.
    // Attempting to check current user on load
    ...
    // Cleanup not typically required for singleton widget, but good hygiene
    return () => {
  ...
  const handleLogin = (tab = 'login') => {
    if (tab === 'guest') {
      ...
      // Open Netlify Widget
      netlifyIdentity.open(tab); // 'login' or 'signup'
    }
  };
